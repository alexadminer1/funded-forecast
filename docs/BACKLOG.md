...
- UI на /checkout не реагирует на CONFIRMED после возвращения юзера → создаёт новый AWAITING (артефакт) — закрывается в Step 5
...
| # | Задача | Детали |
|---|---|---|
| 1 | **On-chain Step 5** — Activation flow | Когда Payment.status=CONFIRMED → создание Challenge (initial balance из plan.accountSize) + recordAffiliateConversionFromPayment. Redirect /checkout → /dashboard. На /checkout логика: если у юзера есть CONFIRMED Payment без активного Challenge — показать "Activating..." вместо создания нового invoice. ~1 час. |
| 2 | Auto-pass logic | При profit target + tradingDaysCount ≥ minTradingDays + !drawdownViolated → status=passed автоматически (вместо ручного админа). Трогает горячий trade flow в `sell/route.ts` |
...
On-chain activation:  ░░░░░░░░░░░░░░░░░░░░    0%  (Step 5 — next)
Auto-pass logic:      ░░░░░░░░░░░░░░░░░░░░    0%
