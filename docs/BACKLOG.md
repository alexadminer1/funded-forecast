...
Последнее обновление: 2026-05-06 (после on-chain checkout UI Step 3).
...
| # | Задача | Детали |
|---|---|---|
| 1 | **Checkout UX + zombie cleanup** | (1) Cron `/api/cron/expire-payments` — AWAITING_PAYMENT с истёкшим expiresAt → EXPIRED (иначе partial unique index блокирует новые invoice). (2) `POST /api/payments/[id]/cancel` endpoint. (3) Cancel button на `/checkout`. (4) Banner на `/account/plans` если есть pending payment. (5) Debug 503 "Concurrent invoice creation conflict" — root cause unknown. |
| 2 | **On-chain Step 4** — Watcher service | Отдельный Node.js worker (Coolify Background Service). Batch poll Alchemy каждые 5 sec. Reads ERC-20 Transfer logs для USDC. Обновляет `lastProcessedBlock`. Idempotent через `(chainId, txHash, logIndex) @unique`. Status flow: DETECTED → MATCHED → CONFIRMED. **Самая большая часть Phase 2.** |
| 3 | **On-chain Step 5** — Activation flow | Когда PaymentTransaction.status=CONFIRMED + Payment.confirmationsSeen ≥ 6 + sum(amountUnits) ≥ expectedAmountUnits → Payment.status=CONFIRMED → создание Challenge + recordAffiliateConversionFromPayment. Также: redirect c /checkout → /dashboard. |
| 4 | Auto-pass logic | При profit target + tradingDaysCount ≥ minTradingDays + !drawdownViolated → status=passed автоматически (вместо ручного админа). Трогает горячий trade flow в `sell/route.ts` |
...
On-chain checkout UI: ████████████████████  100%  (Step 3 done, см. P0 #1 UX bugs)
