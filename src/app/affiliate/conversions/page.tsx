...
export default function AffiliateConversionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const data = await apiFetch<any>(`/api/affiliate/conversions?page=${page}&limit=${LIMIT}`);
    setItems(data.items ?? []);
    ...
    <div style={{ minHeight: "100vh", background: "#080c14", color: "#F1F5F9", fontFamily: "'Inter',-apple-system,sans-serif" }}>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "80px 24px 60px" }}>
        ...
        <div style={{ display: "flex", gap: 24, marginBottom: 28 }}>
          ...
          <a href="/affiliate/conversions" style={{ fontSize: 13, fontWeight: 700, color: "#22C55E", textDecoration: "none" }}>Conversions</a>
          <a href="/affiliate/ledger" style={{ fontSize: 13, color: "#475569", textDecoration: "none" }}>Ledger</a>
        </div>

        <div style={{ fontSize: 18, fontWeight: 700, color: "#F1F5F9", marginBottom: 16 }}>Conversions</div>
        ...
        {items.length === 0 && total === 0 ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", background: "#0d1117", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, overflow: "hidden" }}>
              <thead>
                <tr>
                  {["Date", "Status", "Net", "Rate", "Commission", "Pending until", "Approved at", "Paid at"].map(h => (
              ...
              <tbody>
                {items.map((item: any) => {
                  const sc = STATUS_COLOR[item.status] ?? "#475569";
                  return (
                    <tr key={item.id}>
                      ...
                      <td style={{ fontSize: 13, color: "#F1F5F9", padding: "14px 16px", borderTop: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{formatMoney(item.netAmount)}</td>
                      <td style={{ fontSize: 13, color: "#F1F5F9", padding: "14px 16px", borderTop: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{(item.commissionRate * 100).toFixed(1)}%</td>
                      <td style={{ fontSize: 13, color: "#22C55E", padding: "14px 16px", borderTop: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{formatMoney(item.commissionAmount)}</td>
                      <td style={{ fontSize: 13, color: "#94A3B8", padding: "14px 16px", borderTop: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{formatDate(item.pendingUntil)}</td>
                      <td style={{ fontSize: 13, color: "#94A3B8", padding: "14px 16px", borderTop: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{formatDate(item.approvedAt)}</td>
                      <td style={{ fontSize: 13, color: "#94A3B8", padding: "14px 16px", borderTop: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{formatDate(item.paidAt)}</td>
                    </tr>
                  );
                })}
              </tbody>
