...
export async function POST(
  ...
  const reason = typeof body?.reason === "string" && body.reason.trim().length > 0
    ...
    : null;
  if (!reason) {
    return NextResponse.json({ error: "reason_required", message: "reason required" }, { status: 400 });
  }
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const payout = await tx.affiliatePayout.findUnique({
        where: { id: payoutId },
        select: { id: true, status: true, affiliateId: true, amount: true, includedConversionIds: true },
      });
      if (!payout) {
      ...
      await tx.auditLog.create({
        data: {
          ...
          metadata: {
            ...
            adminLabel,
            reason,
            releasedConversions: conversionIds.length,
          },
          ipAddress,
        },
      });
    ...
    if (msg.startsWith("invalid_transition:")) {
      ...
      return NextResponse.json(
        { error: "invalid_transition", message: `Cannot reject payout in status: ${current}`, currentStatus: current },
        { status: 409 },
      );
    }
    console.error("payout_reject_error", e);
